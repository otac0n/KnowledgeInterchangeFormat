...
namespace KnowledgeInterchangeFormat.Tests.Expressions
{
    ...
    public class ReversePartialRelationDefinitionTests
    {
        ...
        {
            var exception = (ArgumentNullException)Record.Exception(() => new ReversePartialRelationDefinition(null, description, parameters, sequenceVariable, sentence));
        ...
        {
            var exception = (ArgumentNullException)Record.Exception(() => new ReversePartialRelationDefinition(constant, description, parameters, sequenceVariable, null));
        ...
        {
            var exception = (ArgumentNullException)Record.Exception(() => new ReversePartialRelationDefinition(constant, description, null, sequenceVariable, sentence));
            Assert.Equal("parameters", exception.ParamName);
        }

        [Theory]
        [MemberData(nameof(ValidArguments))]
        public void Constructor_WhenGivenValidArguments_CreatesAnObjectWithTheSpecifiedProperties(Constant constant, CharacterString description, IndividualVariable[] parameters, SequenceVariable sequenceVariable, Sentence sentence)
        {
            var subject = new ReversePartialRelationDefinition(constant, description, parameters, sequenceVariable, sentence);
