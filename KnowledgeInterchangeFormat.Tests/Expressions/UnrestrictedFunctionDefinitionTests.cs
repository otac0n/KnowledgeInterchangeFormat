...
namespace KnowledgeInterchangeFormat.Tests.Expressions
{
    ...
    public class UnrestrictedFunctionDefinitionTests
    {
        ...
        {
            var exception = (ArgumentNullException)Record.Exception(() => new UnrestrictedFunctionDefinition(null, null, sentences));
        ...
        {
            var exception = (ArgumentNullException)Record.Exception(() => new UnrestrictedFunctionDefinition(constant, null, null));
            Assert.Equal("sentences", exception.ParamName);
        }

        [Theory]
        [MemberData(nameof(ValidArguments))]
        public void Constructor_WhenGivenValidSentences_CreatesASentenceWithTheSpecifiedSentencesAndConsequents(Constant constant, CharacterString description, Sentence[] sentences)
        {
            var subject = new UnrestrictedFunctionDefinition(constant, description, sentences);
        ...
        {
            var expected = $"(deffunction {constant}{(description == null ? "" : $" {description}")}{string.Concat(sentences.Select(s => string.Concat($" {s}")))})";
            var subject = new UnrestrictedFunctionDefinition(constant, description, sentences);
