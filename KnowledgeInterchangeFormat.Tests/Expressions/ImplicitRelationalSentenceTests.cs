...
namespace KnowledgeInterchangeFormat.Tests.Expressions
{
    ...
    public class ImplicitRelationalSentenceTests
    {
        ...
        {
            var exception = (ArgumentNullException)Record.Exception(() => new ImplicitRelationalSentence(null, arguments, sequenceVariable));
            Assert.Equal("relation", exception.ParamName);
        }

        [Theory]
        [MemberData(nameof(ValidArgumentsNoArgs))]
        public void Constructor_WhenGivenNullArguments_ThrowsArgumentNullException(Constant relation, SequenceVariable sequenceVariable)
        {
            var exception = (ArgumentNullException)Record.Exception(() => new ImplicitRelationalSentence(relation, null, sequenceVariable));
            Assert.Equal("arguments", exception.ParamName);
        }

        [Theory]
        [MemberData(nameof(ValidArguments))]
        public void Constructor_WhenGivenValidSentences_CreatesASentenceWithTheSpecifiedSentencesAndConsequents(Constant relation, IndividualVariable[] arguments, SequenceVariable sequenceVariable)
        {
            var subject = new ImplicitRelationalSentence(relation, arguments, sequenceVariable);
        ...
        [Theory]
        [MemberData(nameof(ValidArguments))]
        public void ToString_Always_ReturnsExpectedOutput(Constant relation, IndividualVariable[] arguments, SequenceVariable sequenceVariable)
        {
            var expected = $"({relation}{string.Concat(arguments.Cast<Variable>().Concat(new[] { sequenceVariable }.Where(v => v != null)).Select(arg => $" {arg}"))})";
            var subject = new ImplicitRelationalSentence(relation, arguments, sequenceVariable);
